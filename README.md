...
* hash 表的查找，平均的时间复杂度是 $O(1)$，而红黑树的查找，平均的时间复杂度是 $O(log_2⁡n )$，TreeNode 占用空间也比普通 Node 的大，如非必要，尽量还是使用链表
...
**数据错乱（1.7，1.8 都会存在）**
 * 链表中相同哈希的数据（比如：a和1的两次哈希（97）都相同，桶下标（1（在16长度数组容量中））相同）
 * 在多线程修改时可能出现前一个线程的节点被后一个线程所覆盖
...
* 散列公式为（第一个字符所表现的数字乘以31的字符串长度-1次幂 第二个 之和）： 
* $S_0∗31^{(n-1)}+ S_1∗31^{(n-2)}+ … S_i ∗ 31^{(n-1-i)}+ …S_{(n-1)}∗31^0$
* 31 是质数可以分布较为均匀 代入公式有较好的散列特性，并且 31 * h 可以被优化为 
  * 31乘以一个数字等于2的5次幂32乘以一个数字减去这个数字
  * 即 $32 ∗h -h $
  * 即 $2^5  ∗h -h$
  * 即 $h≪5  -h$
...
这种方法使用了对象的内存地址来计算哈希值，
  * 对于相同的对象，它们的属性值相同，它们的哈希值也是可能不同的。（可能性不大）
  * 对于地址不同的对象，即使它们的属性值相同，它们的哈希值应该是不同的。（大概率）
  * 基于对象的内存地址来计算哈希值，因此它的运算速度很快，比较高效。但是这样计算出来的哈希值不一定有很好的分布性。
  * 基于对象的内存地址来计算哈希值，所以可能会导致哈希冲突，使得哈希表中的某些桶存放的元素过多， 而导致查询时间变长。
  * 而如果使用其他类型的哈希函数，就可以更好的适用于不同的数据类型,有更好的分布性。

因此，在实际应用中，建议重写hashCode()方法，使用更好的哈希函数来提高哈希表的性能。
  * 如果想要更好的分布性，通常需要重写 hashCode 方法，使用一些算法来把对象的属性值映射到一个整数上。
  * 例如：Objects.hash(name, age);
**一般来说可以用如下算法来重写hashCode**
   * 使用对象中所有参与equals比较的属性的hashcode计算结果的异或和
   * 使用对象中的某些属性的hashcode计算结果的加权和
